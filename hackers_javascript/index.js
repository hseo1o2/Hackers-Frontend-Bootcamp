...
let endTime = new Date(targetDate).getTime() // getTime(): 표준 시에 따라 지정된 날짜의 시간에 해당하는 숫자 값을 반환
let period = endTime - new Date().getTime() // 타깃 날짜에서 오늘 날짜를 빼 카운트다운의 기간을 설정
...
function countDown(){
    period = endTime - new Date().getTime() // 밀리세컨드 반환


    // 정적 Math.floor 메서드: 항상 반올림하여 주어진 숫자보다 작거나 같은 가장 큰 정수를 반환  
